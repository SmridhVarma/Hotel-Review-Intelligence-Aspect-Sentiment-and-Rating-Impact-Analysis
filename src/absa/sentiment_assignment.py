...
def load_aspect_dictionary() -> dict:
    """Load the Stage 2 aspect keyword dictionary."""
    with open(ASPECT_DICTIONARY, encoding="utf-8") as f:
        return json.load(f)


# 836,795 rows — sentences with source_polarity ("positive"/"negative") and reviewer_segment.
# Columns: review_id, hotel_name, reviewer_score, reviewer_segment,
#          review_date, sentence, source_field, source_polarity
df = load_sentences()

# { aspect: [keyword, ...] } — 6 keys: cleanliness, staff, location, noise, food, room
# Available after Stage 2 (aspect_extraction.py) has been run.
aspect_dict = load_aspect_dictionary() if os.path.isfile(ASPECT_DICTIONARY) else None
